...
// Both arguments must be positive. The bucket is initially full.
...
// rateMargin specifes the allowed variance of actual rate from specified rate. 1% seems reasonable.
...
// We grow the quantum exponentially, but slowly, so we get a good fit in the lower numbers.
...
// NewBucketWithQuantum is similar to NewBucket, but allows he specification of the quantum size - quantum tokens are added every fillInterval.
...
// If clock is nil, the system clock will be used.
...
// WaitMaxDuration is like Wait except that it will only take tokens from the bucket if it needs to wait for no greater than maxWait.
...
// It returns the time that the caller should wait until the tokens are actually available.
//
